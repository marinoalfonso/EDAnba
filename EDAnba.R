library(BasketballAnalyzeR) #per grafici
library(ggplot2) #per grafici
library(dplyr) #per data manipulation
library(measurements) #per conversione unita di misura
library(tidyr) #per data manipulation
library(fmsb) #radarchart
library(ggrepel)
library(stringr)

#Descrizione dell'obiettivo del progetto, di quello cui si parla e delle modalità.
#La prima parte si concentrerà sull'analisi descrittiva dei dataset,
#evidenziando gli aspetti che hanno contraddistinto ogni decade
...
nba = read.csv("/Users/alfonsomarino/Desktop/UNISA TRIENNALE/TESI/seasons_stats.csv")
player_stat = read.csv("/Users/alfonsomarino/Desktop/UNISA TRIENNALE/TESI/player_data.csv")
...
#adjustes dataset with TOT team
nba_withTOT = nba %>%
...
#line chart
ggplot(transfer, aes(x = Year, y = n)) +
...
#ANNI 80
#barchart per visualizzazione migliori passatori
tot_ast <- nba_withTOT %>% 
  group_by(Player) %>% 
...
#anni 90
#tabella riguardante statistiche per partita giocatori Bulls
nba %>% 
